...
# nemo guardrails functionality and leverage this as an external plugin server to be leveraged by the ext-proc
# plugin adapter.
...
class NemoWrapperPlugin(Plugin):
    ...
    def __init__(self, config: PluginConfig) -> None:
        ...
        nemo_config = RailsConfig.from_path(os.path.join(os.getcwd(), "plugins/examples/nemo", "pii_detect_config"))
    ...
    async def tool_pre_invoke(self, payload: ToolPreInvokePayload, context: PluginContext) -> ToolPreInvokeResult:
        ...
        logger.warning("[NemoWrapperPlugin] about to detect")
        rails_response = None
        if payload.args:
            try:
                p = payload.args
